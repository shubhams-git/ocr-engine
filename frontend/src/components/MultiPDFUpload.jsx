...
import { Upload, FileText, AlertCircle, X, Plus, BarChart3 } from 'lucide-react'
    ...
    if (rejectedFiles.length > 0) {
      ...
      if (rejectedFile.errors) {
        const errors = rejectedFile.errors.map(error => {
          switch (error.code) {
            case 'file-too-large':
              return 'File size too large. Maximum size is 50MB per PDF.'
            case 'file-invalid-type':
              return 'Only PDF files are supported for multi-document analysis.'
            case 'too-many-files':
              return 'Too many files. Maximum is 10 PDF files.'
            default:
  ...
  return (
    ...
    >
      ...
      >
        ...
        <h2>Multi-PDF Analysis</h2>
        <p>Upload 2-10 PDF documents for comprehensive data analysis, normalization, and projections</p>
      </motion.div>
        ...
        >
          ...
          <p className="dropzone-subtitle">
            {isDragActive
              ...
              : selectedFiles.length > 0
                ...
                : 'Drag & drop 2-10 PDF files here, or click to browse'
            }
          </p>
          ...
          >
            <div className="format-group">
              <FileText size={16} />
              <span>PDF Documents Only (max 50MB each)</span>
            </div>
          </motion.div>
        </motion.div>
      </motion.div>
          ...
          >
            {selectedFiles.length < 2 && (
              <div className="upload-hint">
                <AlertCircle size={16} />
                <span>Add at least 2 PDF files for comparative analysis (up to 10 files)</span>
              </div>
            )}
            ...
            <div className="action-buttons">
              ...
              <motion.button
                ...
                whileHover={{ scale: selectedFiles.length >= 2 ? 1.05 : 1 }}
                whileTap={{ scale: selectedFiles.length >= 2 ? 0.95 : 1 }}
              >
                <BarChart3 size={18} />
                Analyze Documents
              </motion.button>
